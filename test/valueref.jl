...
@assert Vega.tojs(VegaValueRef(value = 5)) == {"value" => 5}

# The value of data.price, for the current datum
@assert Vega.tojs(VegaValueRef(field = "data.price")) == {"field" => "data.price"}

# The value of index for the current datum, multiplied by 20
@assert Vega.tojs(VegaValueRef(field = "index", mult = 20)) == {"field" => "index", "mult" => 20}

# The result of running the value 0 through the scale named x
@assert Vega.tojs(VegaValueRef(scale = "x", value = 0)) == {"scale" => "x", "value" => 0}

# The result of running data.price for the current datum through the scale named y
@assert Vega.tojs(VegaValueRef(scale = "y", field = "data.price")) == {"scale" => "y", "field" => "data.price"}

# The range band width of the ordinal scale x. Note that the scale must be ordinal!
@assert Vega.tojs(VegaValueRef(scale = "x", band = true)) == {"scale" => "x", "band" => true}
